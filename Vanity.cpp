...
Point _2Gn;
std::string formatKeyRate(double keyRate);
// ----------------------------------------------------------------------------
...
std::string toLower2(const std::string& input) {
    std::string result = input; // Cria uma cópia da string original
...
void VanitySearch::output(string addr,string pAddr,string pAddrHex) {
  ...
  if (startPubKeySpecified) {
    ...
    std::string lowercasedHex = toLower2(paddedHex); // Converte para minúsculas
...
void VanitySearch::Search(int nbThread,std::vector<int> gpuId,std::vector<int> gridSize) {
  ...
  while (isAlive(params)) {
    ...
    if (isAlive(params)) {
			printf("\r[%s][Total 2^%.2f]%s[Found %d]  ",
				formatKeyRate(avgKeyRate).c_str(),
				log2((double)count),
...
std::string formatKeyRate(double keyRate) {
    const char* units[] = {"keys/s", "Kkeys/s", "Mkeys/s", "Gkeys/s", "Tkeys/s", "Pkeys/s"};
    int unitIndex = 0;

    // Ajustar unidade com base no valor de keyRate
    while (keyRate >= 1000.0 && unitIndex < 5) {
        keyRate /= 1000.0;
        ++unitIndex;
    }

    char formattedRate[50];
    snprintf(formattedRate, sizeof(formattedRate), "%.2f %s", keyRate, units[unitIndex]);

    return std::string(formattedRate);
}
