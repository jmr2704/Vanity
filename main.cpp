...
void getKeySpace(const string& text, BITCRACK_PARAM* bc, Int& maxKey)
{
    ...
    try
    {
        ...
        // Se a chave inicial estiver vazia, define a chave inicial como 1
        if (item.length() == 0)
        {
        ...
        {
            fprintf(stderr, "Usando maxKey como chave final.\n");
...
void outputAdd(string outputFile, int addrType, string addr, string pAddr, string pAddrHex) {
  ...
  std::string lowercasedHex = toLower(paddedHex); // Converte para minúsculas
